...
| T10 | UI `/conexoes`: adicionar select "Agente default" por conexao (usa `setConnectionAgent`) + toggle `ignore_groups`. | AC4 | edit `src/routes/_shell.conexoes.tsx` | Playwright smoke; persiste via server function. | proof-pending |
| T11 | Prova P4: Playwright smoke `/agentes` e `/conexoes`, `supabase--linter` limpo, `proof.md` escrito. Curl E2E do webhook fica pendente (necessita OPENAI_API_KEY em runtime). | AC1-AC10 | `ldk/features/f3-runtime-agente/proof.md` | Ver `proof.md`. | proof-pending |
