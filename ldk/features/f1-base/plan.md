...
| T4 | Layout com sidebar HUD: `src/routes/_shell.tsx` (pathless layout com `<Outlet/>` + `SidebarProvider` + `SidebarTrigger`), componente `src/components/app-sidebar.tsx` usando `shadcn/ui sidebar` + lucide + tokens F0. Item ativo destacado. Home `/` movida para `src/routes/_shell.index.tsx` (dashboard placeholder dentro do shell) - substitui o redirect planejado para evitar conflito com pathless layout sem children. | AC4, AC5 | `src/routes/_shell.tsx`, `src/routes/_shell.index.tsx`, `src/components/app-sidebar.tsx` | Preview: screenshot com sidebar HUD e dashboard renderizados em `/` | proof-pending |
