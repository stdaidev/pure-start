...
| T6 | UI `/conexoes`: lista + modal nova conexao com QR + polling `useQuery` refetchInterval 3s + acoes refresh/deletar. Badge HUD de status. | AC2, AC3, AC4, AC6 | `src/routes/_shell.conexoes.tsx`, `src/components/conexoes/*` | Playwright: abre `/conexoes`, cria conexao, QR renderiza; screenshot. | ready |
