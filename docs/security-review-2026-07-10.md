...
1. **Critico - autenticacao/autorizacao do painel:** nao ha login nem ownership de usuario. Proteger a implantacao por
   acesso da plataforma/rede ate implementar auth, membership de workspace e policies correspondentes.
...
4. **Alto - cotas de campanha concorrentes:** a reserva por conexao e atomica, mas os contadores por campanha ainda
   sao atualizados no worker. Nao executar ticks concorrentes ate existir uma reserva atomica tambem por campanha.
5. **Medio - LGPD:** definir retencao, exportacao e exclusao de contatos, mensagens e payloads historicos.
