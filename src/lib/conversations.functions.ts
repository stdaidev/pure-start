...
export const listConversations = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const previews = new Map<
    string,
    { content: string | null; created_at: string; direction: string; media_type: string | null }
  >();
  if (ids.length > 0) {
  ...
  .validator((raw: unknown) =>
    z
      .object({
        ...
        lead_value_cents: z.number().int().min(0).max(1_000_000_00).nullable().optional(),
        lead_value_note: z.string().max(280).nullable().optional(),
        lead_outcome: z.union([z.literal("won"), z.literal("lost"), z.null()]).optional(),
      })
      .parse(raw),
  )
  .handler(async ({ data }) => {
...
export const sendConversationMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error: updateErr } = await supabaseAdmin
      ...
      .eq("id", conv.id)
      .eq("workspace_id", DEFAULT_WORKSPACE);
    if (updateErr) console.error("[conversations] last_message_at update failed", updateErr.code);
...
export const deleteConversation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // A FK de messages usa ON DELETE CASCADE: uma unica operacao evita estado parcial.
